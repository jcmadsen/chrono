...
// Include rigid bodies for the shock strut, connect to chassis with hooke joints.
//
// The suspension subsystem is modeled with 
// X pointing towards the front, Y to the left, and Z up (ISO standard).
...
#ifndef CH_MULTILINK_STRUT_H
#define CH_MULTILINK__STRUT_H
...
/// the right side.
///
class CH_VEHICLE_API ChMultiLinkStrut : public ChSuspension
{
public:
  ...
  virtual ~ChMultiLinkStrut() {}

  /// Specify whether or not this suspension can be steered.
  virtual bool IsSteerable() const { return false; }
  ...
  virtual void Initialize(
    ...
    ChSharedPtr<ChBody>        tierod_body_left,  ///< [in] left tierod body connection
	ChSharedPtr<ChBody>		   tierod_body_right  ///< [in] right tierod body connection
    );
...
protected:
  ...
  /// return lower strut mass
  virtual double getLowerStrutMass() const = 0;
  /// return upper strut mass
  virtual double getUpperStrutMass() const = 0;
  ...
  /// return lower strut radius
  virtual double getLowerStrutRadius() const = 0;
  /// return upper strut radius
  virtual double getUpperStrutRadius() const = 0;
  ...
  ChSharedBodyPtr m_lowerStrut[2]; ///< handle to lower strut body (L/R)
  ChSharedBodyPtr m_upperStrut[2]; ///< handle to upper strut body (L/R)
  ...
  ChSharedPtr<ChLinkUniversal>      m_hookeLateralChassis[2]; ///< handles to the chassis-lateral universal joints (left/right)
  ChSharedPtr<ChLinkLockSpherical>  m_sphericalLateralUpright[2]; ///< handles to the upright-lateral spherical joints (left/right)
  ChSharedPtr<ChLinkUniversal>      m_hookeTLChassis[2];      ///< handles to the chassis-trailing link universal joints (left/right)
